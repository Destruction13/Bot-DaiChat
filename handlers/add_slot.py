...
async def date_chosen(
    ...
    if selected:
        ...
        await callback.message.answer(
            escape_md("Введите время слота в формате ЧЧ:ММ - ЧЧ:ММ")
        )
...
async def time_entered(message: Message, state: FSMContext) -> None:
    if not TIME_PATTERN.match(message.text):
        await message.answer(
            escape_md("Неверный формат времени. Используйте ЧЧ:ММ - ЧЧ:ММ")
        )
        return
    ...
    try:
        ...
        await message.answer(escape_md("Некорректное время"))
        return
    await state.update_data(time=f"{start} - {end}")
    await state.set_state(AddSlot.enter_link)
    await message.answer(escape_md("Введите ссылку на встречу"))
...
async def link_entered(message: Message, state: FSMContext) -> None:
    ...
    if not parsed.scheme.startswith("http"):
        await message.answer(escape_md("Некорректная ссылка"))
        return
    ...
    if success:
        await message.answer(
            escape_md("Слот успешно добавлен"), reply_markup=MAIN_MENU
        )
    else:
        await message.answer(
            escape_md("Такой слот уже существует"), reply_markup=MAIN_MENU
        )
    await state.clear()
